    ...
    },
    {
      ...
      render: (_, record) => [
        <Button
          ...
          size="small"
          key="edit"
          hidden={!hasPermiOr(['system:operlog:edit'])}
          onClick={() => {
    ...
    <div>
        ...
        <ProTable<API.Monitor.Operlog>
          ...
          toolBarRender={() => [
            <Button
              type="primary"
              key="add"
              hidden={!hasPermiOr(['system:operlog:add'])}
              onClick={async () => {
            ...
            <Button
              ...
              hidden={
                ...
                !hasPermiOr(['system:operlog:remove'])
              }
              onClick={async () => {
                Modal.confirm({
                  title: '是否确认删除所选数据项?',
            ...
            <Button
              type="primary"
              key="export"
              hidden={!hasPermiOr(['system:operlog:export'])}
              onClick={async () => {
                handleExport();
              }}
            >
      ...
      {selectedRows?.length > 0 && (
        ...
        >
          <Button
            key="remove"
            hidden={!hasPermiOr(['system:operlog:del'])}
            onClick={async () => {
              Modal.confirm({
                title: '删除',
                content: '确定删除该项吗？',
